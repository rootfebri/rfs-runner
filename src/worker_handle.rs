  ...
  F: Fn(D, MainProgress<S>) -> Fut,
{
  ...
  fn update_state(&mut self, output: anyhow::Result<()>) {
...
mod test {
  ...
  #[tokio::test]
  async fn test_capture_owned_var() {
    async fn the_solver_fn(_: String, _: MainProgress<DefaultTemplate>, _: Arc<AtomicBool>) -> anyhow::Result<()> {
      Ok(())
    }
